...
import type { TournamentData } from '../hooks/useTournament';
...
interface TournamentContextType {
  ...
  fetchTournaments: (status?: string) => Promise<TournamentData[]>;
  createTournament: (
    name: string,
    format: string,
    entryFee?: number,
...
export const TournamentProvider: React.FC<TournamentProviderProps> = ({ children }) => {
  ...
  // tournaments state is loaded from fetchTournaments into `tournaments`
  ...
  const createTournament = useCallback(async (
    ...
    try {
      await createTournamentHook(name, format, entryFee, maxPlayers);
  ...
  const joinTournament = useCallback(async (tournamentId: number): Promise<boolean> => {
    try {
      await joinTournamentHook(tournamentId);
  ...
  const startTournament = useCallback(async (tournamentId: number): Promise<boolean> => {
    try {
      await startTournamentHook(tournamentId);
