  ...
  const joinGame = useCallback(async (roomId: number) => {
    try {
      ...
      if (gameState) {
        ...
        const isOurTurn = gameState.currentPlayer === address;
    ...
    try {
      ...
      if (result) {
        ...
        if (currentGame) {
          ...
          newBoard[position] = address ? 'X' : 'O'; // Simplified
          setCurrentGame({
            ...currentGame,
            board: newBoard,
            currentPlayer: address, // Toggle player
            movesHistory: [...currentGame.movesHistory, position]
          });
